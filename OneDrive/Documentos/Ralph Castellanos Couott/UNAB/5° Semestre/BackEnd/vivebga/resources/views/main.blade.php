...
<div class="container" style="max-width: 1200px; margin: 0 auto; padding: 20px;">
    ...
    {{-- Definimos las categorías oficiales directamente en la vista como fallback --}}
    @php
        $categoriasOficiales = $categorias ?? [
            'Música',
            'Deportes',
            'Arte',
            'Tecnología',
    ...
    {{-- Barra de búsqueda con filtro de categorías --}}
    <form action="{{ route('event.search') }}" method="GET" class="mb-4 d-flex justify-content-center">
        <input type="text" name="query" class="form-control me-2" placeholder="Buscar eventos u organizadores..." 
               value="{{ request('query') }}" style="max-width: 300px;">

        <select name="categoria" class="form-select me-2" style="max-width: 200px;">
            <option value="">Todas las categorías</option>
            @foreach($categoriasOficiales as $cat)
        ...
        <div class="eventos-grid"
            ...
            @foreach ($eventos as $evento)
                <div class="evento-card"
                    ...
                    @else
                        <div style="width: 100%; height: 180px; background: #eee; display: flex; align-items: center; justify-content: center; border-radius: 8px; margin-bottom: 10px;">
